...
document = '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'+html.escape(title)+'</title><style>'+css+'</style></head><body><div class="layout"><aside><b>Guard 使用说明</b><br><span class="tag">'+html.escape(revision)+' · 文档日期</span><nav>'+''.join(navigation)+'</nav><p><button onclick="window.print()">打印 / 保存 PDF</button></p></aside><main>'+''.join(out)+'<footer>fuck-anthropic guard · Local-first · 无外部字体、脚本或追踪资源<br>后续更新请维护 Markdown 源文件并运行 scripts/render-guide.py。</footer></main></div></body></html>'
