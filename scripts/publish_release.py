...
if a.prepare:
 ...
 app=R/'dist/release/Claude Connection Watcher.app'
 run(str(app/'Contents/MacOS/ClaudeConnectionWatcher'),'--self-test')
 archive=release/'ClaudeConnectionWatcher-0.4.0-beta.1.zip'
 def pack():
...
if not a.source_only:
 run('gh','release','create',a.tag,str(archive),'--target',m['source_commit'],'--prerelease','--latest=false','--title','Watcher 0.4.0 beta — Surge companion','--notes-file','docs/RELEASE-NOTES.md')
